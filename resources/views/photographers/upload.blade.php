    ...
    <div class="container-fluid" id="photo-uploader"
        ...
        <div class="d-flex flex-wrap justify-content-between align-items-start gap-3 mb-4">
            <div>
                ...
                <p class="text-muted">{{ $event->date_label }} ·
                    {{ $event->venue_name ? $event->venue_name . ' · ' : '' }}{{ $event->location_label }}</p>
            </div>
            <div class="text-md-end">
                ...
                <div class="small mt-2"><strong>Upload
                        deadline:</strong><br>{{ $deadline->timezone($event->timezone)->format('M j, Y g:i A T') }}</div>
            </div>
        </div>
        <p class="small text-muted mb-4">Queued: {{ $counts['queued'] ?? 0 }} · Uploading: {{ $counts['uploading'] ?? 0 }}
            · Processing: {{ $counts['processing'] ?? 0 }} · Ready: {{ $counts['ready'] ?? 0 }} · Rejected:
            {{ $counts['rejected'] ?? 0 }} · Published: {{ $counts['published'] ?? 0 }}</p>
        ...
        <div class="row g-3 mb-4">
            @foreach ([['Accepted', $acceptedCount . ' / ' . $rules['max_event_photos'], 'primary'], ['Remaining', $remainingCount, 'secondary'], ['Ready', $counts['ready'] ?? 0, 'success'], ['Rejected', $counts['rejected'] ?? 0, 'danger'], ['Published', $counts['published'] ?? 0, 'info']] as [$label, $value, $color])
                <div class="col-6 col-lg">
                    <div class="card h-100">
                        <div class="card-body py-3">
                            <div class="small text-muted">{{ $label }}</div>
                            <div class="h4 mb-0 text-{{ $color }}" data-count="{{ strtolower($label) }}">
                                {{ $value }}</div>
                        </div>
                    </div>
                </div>
        ...
        <div class="card mb-4">
            <div class="card-header bg-white">
                <h2 class="h5 mb-0">Photo requirements</h2>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-lg-6">
                    ...
                    <div class="col-lg-6">
                        ...
                        @if ($assignment->rights_confirmed_at)
                            <div class="alert alert-success py-2 mb-0">Rights and backup confirmed
                                {{ $assignment->rights_confirmed_at->format('M j, Y') }}.</div>
                        ...
                        @else
                            <div class="form-check border rounded p-3 ps-5">
                                ...
                                <label class="form-check-label" for="rights-confirmed">I confirm that I own or control the
                                    rights to these photographs, may offer them for sale, and have kept my own
                                    backup.</label>
                            </div>
                        @endif
                    </div>
        ...
        <div class="d-flex flex-wrap align-items-center gap-2 mb-2">
            ...
            <div class="form-check"><input class="form-check-input" id="select-all-ready" type="checkbox"
        ...
        <p class="small text-muted mb-3">Add a natural title, useful alt text, and a caption. Identify people only when you
            are authorized to publish their names; minors require special care.</p>
                ...
                <div class="col-6 col-md-4 col-xl-3" data-server-photo="{{ $photo->uuid }}">
                    <div class="card h-100">
                        @if (in_array($photo->status, ['ready', 'published']))
                            <img src="{{ route('photographer.uploads.preview', [$event, $photo]) }}" class="card-img-top"
                                style="aspect-ratio:1/1;object-fit:cover" alt="">
                        @endif
                        <div class="card-body p-3">
                            <div class="text-truncate small" title="{{ $photo->original_filename }}">
                                {{ $photo->original_filename }}</div>
                            <span
                                class="badge mt-2 {{ $photo->status === 'rejected' ? 'bg-danger' : ($photo->status === 'ready' ? 'bg-success' : 'bg-secondary') }}">{{ ucfirst($photo->status) }}</span>
                            @if ($photo->rejection_reason)
                                <p class="small text-danger mt-2 mb-0">{{ $photo->rejection_reason }}</p>
                            @endif
                            @if ($photo->status === 'ready')
                                <div class="form-check mt-2"><input class="form-check-input" type="checkbox"
                                        name="photo_ids[]" value="{{ $photo->uuid }}" form="publish-photos-form"
                                        checked><label class="form-check-label small">Publish</label></div>
                            @endif
                            ...
                            @if (in_array($photo->status, ['ready', 'published']))
                                <details class="mt-3" open>
                                    ...
                                    <form method="POST"
                                        action="{{ route('photographer.uploads.metadata', [$event, $photo]) }}"
                                        class="mt-3">
                                        @csrf
                                        @method('PATCH')
                                        <label class="form-label small" for="title-{{ $photo->uuid }}">Photo
                                            title</label>
                                        <input class="form-control form-control-sm mb-2" id="title-{{ $photo->uuid }}"
                                            name="title" maxlength="80" value="{{ $photo->title }}"
                                            placeholder="Runner crossing the finish line">

                                        <label class="form-label small" for="alt-{{ $photo->uuid }}">Alt text</label>
                                        <textarea class="form-control form-control-sm mb-2" id="alt-{{ $photo->uuid }}" name="alt_text" maxlength="250"
                                            rows="2" placeholder="Describe what is visible">{{ $photo->alt_text }}</textarea>

                                        <label class="form-label small" for="caption-{{ $photo->uuid }}">Caption</label>
                                        <textarea class="form-control form-control-sm mb-2" id="caption-{{ $photo->uuid }}" name="caption"
                                            maxlength="1000" rows="2">{{ $photo->caption }}</textarea>

                                        <label class="form-label small" for="copyright-{{ $photo->uuid }}">Copyright
                                            notice</label>
                                        <input class="form-control form-control-sm mb-2"
                                            id="copyright-{{ $photo->uuid }}" name="copyright_notice" maxlength="255"
                                            value="{{ $photo->copyright_notice }}" placeholder="© 2026 Copyright owner">

                                        <label class="form-label small" for="people-{{ $photo->uuid }}">People
                                            shown</label>
                                        <textarea class="form-control form-control-sm mb-2" id="people-{{ $photo->uuid }}" name="people" maxlength="2000"
                                            rows="2" placeholder="One name per line">{{ collect($photo->people)->join("\n") }}</textarea>

                                        <div class="form-check mb-2">
                                            <input class="form-check-input" id="people-confirmed-{{ $photo->uuid }}"
                                                name="people_publication_confirmed" type="checkbox" value="1"
                                                @checked($photo->people_publication_confirmed_at)>
                                            <label class="form-check-label small"
                                                for="people-confirmed-{{ $photo->uuid }}">I confirm authorization to
                                                publish these names, including guardian authorization for any minor.</label>
                                        </div>
                                        <button class="btn btn-sm btn-outline-primary" type="submit">Save SEO
                                            details</button>
                                    </form>
                                </details>
                            @endif
                            ...
                            @if (in_array($photo->status, ['queued', 'uploading', 'ready', 'rejected']))
                                <form method="POST"
                                    action="{{ route('photographer.uploads.destroy', [$event, $photo]) }}"
                                    class="mt-2">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="card mt-4 border-info">
            <div class="card-body">
                ...
                @if ($event->gallery_published_at)
                    <p class="mb-1"><strong>Gallery published:</strong>
                        {{ $event->gallery_published_at->timezone($event->timezone)->format('M j, Y g:i A T') }}</p>
                    <p class="mb-1"><strong>Sales close / unsold media deletion:</strong>
                        {{ $event->sales_close_at->timezone($event->timezone)->format('M j, Y g:i A T') }}
                        ({{ now()->diffInDays($event->sales_close_at, false) }} days remaining)</p>
                ...
                @else
                    <p class="mb-0"><strong>Expected expiration:</strong> 60 days after the first gallery publication. No
                        publication date has been set yet.</p>
                @endif
            </div>
        </div>
    </div>
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            selectAll?.addEventListener('change', () => document.querySelectorAll('input[name="photo_ids[]"]')
            ...
            const refreshOverall = () => {
                ...
                overall.style.width = progress + '%';
                overallLabel.textContent = items.length ?
                    `${progress}% overall · ${items.filter(i => i.progress === 100).length} transferred` :
                    'No batch selected';
            ...
            const selectFiles = fileList => {
                ...
                if (files.length > Number(root.dataset.maxFiles)) return showError(
                    `Select no more than ${root.dataset.maxFiles} photos per batch.`);
            ...
            const putFile = (item, upload) => new Promise((resolve, reject) => {
                ...
                xhr.upload.onprogress = event => {
                    if (event.lengthComputable) {
                        ...
                        item.label = `Uploading ${Math.round(event.loaded / event.total * 100)}%`;
                        render();
                    }
                };
                xhr.onload = () => xhr.status >= 200 && xhr.status < 300 ? resolve() : reject(new Error(
                    'Upload interrupted. Retry this file.'));
            ...
            const uploadOne = async (item, upload) => {
                try {
                    ...
                    item.progress = 95;
                    render();
                    await jsonRequest(root.dataset.completeTemplate.replace('__PHOTO__', item.id), {
                        method: 'POST',
                        body: '{}'
                    });
            ...
            uploadButton.addEventListener('click', async () => {
                clearError();
                if (!document.getElementById('rights-confirmed').checked && document.getElementById(
                        'rights-confirmed').type === 'checkbox') return showError(
                    'Confirm the rights and backup notice before uploading.');
                uploadButton.disabled = true;
                try {
                    const body = {
                        rights_confirmed: true,
                        files: items.filter(i => i.status === 'queued').map(i => ({
                            name: i.file.name,
                            size: i.file.size,
                            type: i.file.type || 'image/jpeg'
                        }))
                    };
                    const batch = await jsonRequest(root.dataset.createUrl, {
                        method: 'POST',
            ...
            retryButton.addEventListener('click', async () => {
                for (const item of items.filter(i => i.status === 'failed' && i.id)) {
                    ...
                    item.progress = 0;
                    try {
                        await uploadOne(item, await jsonRequest(root.dataset.retryTemplate.replace(
                            '__PHOTO__', item.id), {
                            method: 'POST',
                            body: '{}'
                        }));
