...
@section('content')
    <main class="container py-5">
        ...
        @if (!$isPaid)
            <div class="alert alert-info" role="status">Your payment has not been confirmed yet. This page will show your
                downloads once it is.</div>
        @else
                        ...
                        <div class="card h-100">
                            @if ($thumbnailUrls[$item->id] ?? null)
                                <img class="card-img-top" style="aspect-ratio: 1 / 1; object-fit: cover;"
                                    src="{{ $thumbnailUrls[$item->id] }}" alt="Purchased photo">
                            @endif
                            <div class="card-body p-2 text-center">
                                @if ($downloadableItemIds->contains($item->id))
                                    <a class="btn btn-sm btn-primary w-100"
                                        href="{{ route('orders.download', ['accessToken' => $order->access_token, 'photo' => $item->photo_uuid]) }}">Download
                                        original</a>
                                @else
